...
- **[claude-handoff](./claude-handoff/SKILL.md)**: Save a conversation handoff; launch a Claude background agent only on explicit user request. User-invoked.
...
- **[implement-spec](./implement-spec/SKILL.md)**: Implement a whole spec on one branch. Works the tickets as a task graph rather than a list, working ready tickets sequentially by default, with subagents only on explicit user request, and lands the result as a single PR. User-invoked.
