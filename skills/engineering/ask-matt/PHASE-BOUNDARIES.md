...
The **phase boundary** is the gap between two phases. Prefer assessing context needs there; continue mid-phase work from its existing state. Delegate only when the user explicitly requests multi-agent work for this task. A delegated agent completes its assigned task directly.
...
| **Subagent** | Delegate an independent task only on explicit user request.     |
...
**1. Can you continue in this session?** Continue when the next phase benefits from the current reasoning and the harness has enough context available. Use actual context limits and task needs rather than a fixed token threshold for every model. Grilling → implementation often benefits from retaining the decisions and their reasons.
...
**4. Has the user explicitly requested delegation?** If so, and an independent task can run without steering, a **subagent** can return a bounded report. Otherwise keep the task in the current agent; being suitable for unattended work does not itself authorize delegation.
